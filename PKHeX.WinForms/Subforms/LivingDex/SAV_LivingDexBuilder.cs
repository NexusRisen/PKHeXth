...
public partial class SAV_LivingDexBuilder : Form
{
    ...
    {
        InitializeComponent();
        // Translation will be handled by PKHeX if needed
        SAV = sav;
        Trainers = trainers;
    ...
    private byte GetMinimumLevelForSpecies(ushort species, byte form)
    {
        ...
        if (node.First.Species == 0)
            return 1; // Experience.MinLevel // Base form
        ...
        foreach (var method in methods.Span)
        {
            ...
            {
                if (method.LevelUp > 0)
                    return (byte)Math.Max((int)method.LevelUp, 1); // Experience.MinLevel
    ...
    private void B_Generate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                MessageBox.Show("No missing entries to generate!", "Living Dex Builder", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
    ...
    private void DisplayGenerationResults(int requestedCount)
    {
        ...
        if (GeneratedPokemon.Count > 0)
        {
            ...
            MessageBox.Show(message, "Living Dex Builder", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        else
        {
            MessageBox.Show("No valid Pokémon could be generated with the current settings.", "Living Dex Builder", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
    }
    ...
    private List<IEncounterInfo> SearchForEncounters(PKM pk)
    {
        ...
        var moves = ReadOnlyMemory<ushort>.Empty;
        var encounters = EncounterMovesetGenerator.GenerateEncounters(pk, moves, versions.ToArray())
    ...
    private void B_Import_Click(object sender, EventArgs e)
    {
        if (GeneratedPokemon.Count == 0)
        {
            MessageBox.Show("No Pokémon to import!", "Living Dex Builder", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        {
            MessageBox.Show($"Ran out of box space! Imported {imported}/{GeneratedPokemon.Count} Pokémon.", "Living Dex Builder", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        else
        {
            MessageBox.Show($"Successfully imported {imported} Pokémon starting from Box {NUD_StartBox.Value}!", "Living Dex Builder", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
