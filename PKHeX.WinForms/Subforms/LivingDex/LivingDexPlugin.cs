...
public class LivingDexPlugin : IPlugin
{
    ...
    private void AddToDataMenu(ToolStripDropDownItem dataMenu)
    {
        ...
        for (int i = 0; i < dataMenu.DropDownItems.Count; i++)
        {
            ...
            if (item != null && (item.Name == "Menu_BatchEditor" ||
                (item.Text?.Contains("Batch Editor") ?? false)))
            {
                batchEditorIndex = i;
                break;
            }
    ...
    private static string GetTrainerPath()
    {
        // Get the TrainerPath directly from PKHeX settings
        return Program.Settings.LocalResources.GetTrainerPath();
    }
